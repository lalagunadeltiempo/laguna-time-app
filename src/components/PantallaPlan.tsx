...
export function PantallaPlan({ onOpenInMapa, tab: tabProp, onTabChange }: PantallaPlanProps = {}) {
  ...
  return (
    <div className="flex flex-1 flex-col px-6 py-8">
      ...
      {/* Tab bar */}
      <div className="mb-4 flex gap-1 overflow-x-auto rounded-xl bg-surface p-1" role="tablist" aria-label="Horizontes de planificación">
        {TABS.map((t) => (
          <button
            key={t.id}
            role="tab"
            ...
            className={`min-h-[44px] min-w-0 flex-1 whitespace-nowrap rounded-lg px-2 py-2.5 text-xs font-semibold transition-colors ${
              tab === t.id
      ...
      <div className="mb-6 flex items-center gap-2">
        <button onClick={goBack} className="flex h-10 w-10 items-center justify-center rounded-lg border border-border text-muted transition-colors hover:bg-surface hover:text-foreground" aria-label="Anterior">
        ...
        <button onClick={goForward} className="flex h-10 w-10 items-center justify-center rounded-lg border border-border text-muted transition-colors hover:bg-surface hover:text-foreground" aria-label="Siguiente">
        ...
        <span className="min-w-0 flex-1 truncate text-center text-sm font-semibold text-foreground">{label}</span>
